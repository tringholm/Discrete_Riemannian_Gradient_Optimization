...
#define xtol_in         prhs[10]
#define dt_in           prhs[11]
#define M_in            prhs[12]
#define N_in            prhs[13]
...
void DGstep(double *g, double *u, int *mask, double *dp, double *dqx,
        ...
        double xtol, double dt, int M, int N);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    double gamma, xtol, dt;
    double *g, *u, *dp, *dqx, *dqy, *Suinvprod, *Sginvprod, *Vhist, *maskdbl, *u_out;
    int p, M, N, i;
    int *mask;
    ...
    // ------------------------------------------ evaluate function
    DGstep(g, u, mask, dp, dqx, dqy, Suinvprod, Sginvprod, p, gamma, xtol, dt, M, N);    
    ...
    for (i = 0; i < M; i++){
        for (j = 0; j < N; j++){
            if (mask[i + j*M]){
                ...
                if (i < M-1 && mask[i+1 + j*M]){
                    masks[0] = 1;
                }
                if (i > 0 && mask[i-1 + j*M]){
                    masks[1] = 1;
                }
                if (j < N-1 && mask[i + (j+1)*M]){
                    masks[2] = 1;
                }
                if (j > 0 && mask[i + (j-1)*M]){
                    masks[3] = 1;
...
void prodform(double *V, double *L, double *S){
    double x1, x2, x3, x4, x5, x6, x1x2, x1x3, x2x2, x2x3, x2x4, x2x5, x3x3, 
           x3x5, x3x6, x4x5, x5x5, x5x6, x1x5, x3x4, x2x6, L0V0, L1V3, L2V6,
           L0V1, L1V4, L2V7;
    double X[9];
    L0V0 = L[0]*V[0]; L0V1 = L[0]*V[1]; L1V3 = L[1]*V[3];
    L1V4 = L[1]*V[4]; L2V6 = L[2]*V[6]; L2V7 = L[2]*V[7];

    X[0] = L0V0*V[0] + L1V3*V[3] + L2V6*V[6];
    X[1] = L0V0*V[1] + L1V3*V[4] + L2V6*V[7];
    X[2] = L0V0*V[2] + L1V3*V[5] + L2V6*V[8];
    X[3] = X[1];
    X[4] = L0V1*V[1] + L1V4*V[4] + L2V7*V[7];
    X[5] = L0V1*V[2] + L1V4*V[5] + L2V7*V[8];
    X[6] = X[2];
    X[7] = X[5];
    X[8] = L[0]*V[2]*V[2] + L[1]*V[5]*V[5] + L[2]*V[8]*V[8];
    
    x1 = X[0]; x2 = X[1]; x3 = X[2]; x4 = X[4]; x5 = X[5]; x6 = X[8];
    
    x1x2 = x1*x2;  x1x3 = x1*x3;
    
    x2x2 = x2*x2;  x2x3 = x2*x3;  x2x4 = x2*x4;  x2x5 = x2*x5;
    
    x3x3 = x3*x3;  x3x5 = x3*x5;  x3x6 = x3*x6;
    
    x4x5 = x4*x5;
    
    x5x5 = x5*x5;  x5x6 = x5*x6;
    
    x1x5 = x1*x5 + x2x3;  x3x4 = x3*x4 + x2x5;  x2x6 = x2*x6 + x3x5;
...
double d_M(double *XX, double *A){
    double A1, A2, A3, A4, A5, A6, q, p, d, r, phi, third;
    third = 1.0/3.0;
    ...
    p = sqrt(2.0*(A1*A1 + A4*A4 + A6*A6 + 2.0*p)*third);
